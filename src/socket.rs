...
pub(crate) fn socket_transport(
    stream: TcpStream, exit: Arc<AtomicBool>,
...
fn make_reader(stream: TcpStream, exit: Arc<AtomicBool>) -> (Receiver<Message>, thread::JoinHandle<io::Result<()>>) {
    ...
    let reader_thread = thread::spawn(move || {
        ...
        while let Some(msg) = Message::read(&mut buf_read).unwrap() {
            ...
            if exit.load(Ordering::Relaxed) {
                Logger::info("socket read finished");
                return Ok(());
            }
        }
        Ok(())
...
fn make_writer(mut stream: TcpStream, exit: Arc<AtomicBool>) -> (Sender<Message>, thread::JoinHandle<io::Result<()>>) {
    ...
    let writer_thread = thread::spawn(move || {
        receiver_from_client
            ...
            .try_for_each(|it| {
                ...
                Logger::debug(&format!("socket write {:#?}", &it));
